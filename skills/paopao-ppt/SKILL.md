...
with two product limits:

- Use only the prompt templates present in this public repository.
- Unlicensed/free use is limited to 15 slides per deck.

Do not simplify, reorder, or skip workflow stages because this is the public
edition. Quality depends on the full Image2-first pipeline below.
...
- The required reference images are AI-generated Image2 outputs created before HTML/PPTX reconstruction. PNGs exported from HTML, browser previews, PowerPoint, PDF, or the final PPTX are QA artifacts only; they are never a substitute for the required pre-PPTX Image2 references.
...
- If no user-visible Image2 approval happened earlier in the chat, do not deliver a PPTX. Stop, generate/show the missing Image2 page previews, ask for approval, and only then continue reconstruction.
...
If the user reports that the public plugin output is poor, skipped image generation, or seems to follow an old workflow, tell them briefly: "这两天产品一直在更新，如果现在效果不好，可能过一会儿更新到最新版后会好一些；建议先拉取/重装最新 GitHub 版本再试。" Then continue fixing the actual technical issue.
